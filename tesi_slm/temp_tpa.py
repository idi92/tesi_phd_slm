import numpy as np
from tesi_slm.temp_tpr import load_measures
...
class TiltedPsfAnalyzer():
    ...
    def _get_image_peak_and_coords(self, image):
        peak = image.max()
        ymax, xmax = np.where(image == peak)[
            0][0], np.where(image == peak)[1][0]
    ...
    def compute_tilted_psf_displacement(self, fwhm_x=3.3, fwhm_y=3.3, method='fbyf'):
        ...
        if(method == 'fbyf'):
            ...
            for idx in range(Ntilts):
                ...
                for k_idx in range(Nframes):
                    ...
                    fit = self._gaussian_fit(
                        cut_image, 2, 2, fwhm_x, fwhm_y, peak)
                    par = fit.parameters
                    xk[k_idx] = par[1] + xc
                    yk[k_idx] = par[2] + yc
        ...
        if(method == 'collapse'):
            ...
            for idx in range(Ntilts):
                ...
                fit = self._gaussian_fit(
                    image, xc, yc, fwhm_x, fwhm_y, peak)
                # fit = self._weighted_gaussian_fit(
                #     image, err_ima, xc, yc, fwhm_x, fwhm_y, peak)
                par = fit.parameters
